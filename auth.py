...
SECRET_KEY = os.environ.get("SECRET_KEY", "tu-clave-segura-aqui")  # Idealmente usar variables de entorno
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 120  # Aumentado de 30 a 60 minutos
...
async def authenticate_user(email: str, password: str) -> Optional[UserInDB]:
    ...
    try:
        logger.info(f"Intento de autenticación para: {email}")
        user = get_user(email)
        if not user:
        ...
        logger.error(f"Error HTTP durante autenticación: {e.status_code} - {e.detail}")
        raise e
    except Exception as e:
